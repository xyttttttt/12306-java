...
public class TrainBusinessClassPurchaseTicketHandler extends AbstractTrainPurchaseTicketTemplate {
     ...
     *
     * @param requestParam
     * @param trainCarriageList  车厢
     * @param trainStationCarriageRemainingTicket  车厢余票
     * @return
     */
    private Pair<List<TrainPurchaseTicketRespDTO>, Boolean> findMatchSeats(SelectSeatDTO requestParam, List<String> trainCarriageList, List<Integer> trainStationCarriageRemainingTicket) {
        ...
        /**
         * 该循环用于为指定列车的多个车厢分配座位，处理选座逻辑与自动分配座位。
         * 主要功能包括：
         * 1. 遍历每个车厢，获取该车厢中指定座位类型的可用座位；
         * 2. 构造座位矩阵，标记哪些座位可用；
         * 3. 根据乘客选座请求，尝试分配指定座位；
         * 4. 若指定座位不可用但同列有空位，则尝试分配同列其他座位；
         * 5. 若当前车厢无法满足需求，则记录该车厢空座信息，后续统一处理；
         * 6. 当所有车厢遍历完成后，尝试从已记录的空座中选出满足乘客数的座位；
         * 7. 若单个车厢无法满足，则跨车厢分配座位；
         * 8. 最终返回分配结果和是否成功的标志。
         *
         * 注意：该代码逻辑较为复杂，涉及多个嵌套循环、条件判断及集合操作。
         */
        for (int i = 0; i < trainStationCarriageRemainingTicket.size(); i++) {
            // 获取当前车厢编号
            String carriagesNumber = trainCarriageList.get(i);
            // 查询当前车厢中指定座位类型在出发站到终点站之间的可用座位列表
